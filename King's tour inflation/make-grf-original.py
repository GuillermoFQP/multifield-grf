...
# spectrum parameters
gamma = 1.0; cutoff = 20.0

# random generator instance
seed = None; rng = default_rng(seed)

# random field power spectrum (normalized)
P = np.exp(-K2/(2.0*cutoff**2))/K2**gamma; P[0,0] = 0.0; P /= integrate(P)
...
desc = "King's tour trajectory on hexagonal board"
with open('tour/hex.json', 'r') as catalog:
	tour = json.load(catalog)[0]
nodes = np.array(tour) @ np.array([[1,0],[-0.5,sqrt(0.75)]])/2.5
