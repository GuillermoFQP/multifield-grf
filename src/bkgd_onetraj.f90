...
eta_para = 0.0 ! Initialize slow-roll parameter $\eta_{\parallel}(t_{0})$
eta_perp = 0.0 ! Initialize slow-roll parameter $\eta_{\perp}(t_{0})$
...
do while (condition)
	...
	! Update slow-roll/slow-turn parameters
	epsilon  = epsilon_sr(phi, phidot, H)
	eta_para = eta_sr(phi, phidot, H)
	eta_perp = turnrate(phi, phidot, H)
	
	if (N >= N_flush) then
!		write (10, '(7(6e25.10e3))') phi, potential(phi), N, epsilon
		write (*, '(7(6e25.10e3))') phi, phidot, potential(phi), N, epsilon, eta_para, eta_perp
		N_flush = N_flush + dN
	end if
	...
	! Update condition
	if (epsilon >= 1.0) condition = .false.
end do
